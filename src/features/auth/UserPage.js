...
const UserPage = () => {
    ...
    if (isSuccess && isPostSuccess) {
        ...
        const allUserPosts = [...posts?.ids].filter(post => posts?.entities[post].author === id).sort((a, b) => posts?.entities[b].date - posts?.entities[a].date)

        const userPosts = allUserPosts.map(currentPost => {
            return (
                <div key={posts?.entities[currentPost].searchField}>
                    <img src={posts?.entities[currentPost].thumbnail} alt="post-thumbnail"/>
                    <p>{posts?.entities[currentPost].title}</p>
                </div>
            )
        })
